...
export class PostgresRepository implements CrewRepository {
  ...
  private async receipt(tx: DbTx, actor: string, commandId: string): Promise<{ request_hash: string; response: unknown } | undefined> {
    const rows = await tx.query<Row>(
      `select request_hash, response from public.crew_command_receipts where actor_user_id = $1::uuid and command_id = $2::uuid`,
      [actor, commandId],
    );
  ...
  private async saveReceipt(
    ...
    await tx.query(
      `insert into public.crew_command_receipts
         (actor_user_id, command_id, room_id, request_hash, committed_revision, response)
       values ($1::uuid, $2::uuid, $3::uuid, $4::text, $5::bigint, $6::text::jsonb)`,
  ...
  private async recordEvent(tx: DbTx, roomId: string, revision: number, actor: string, type: string, payload: unknown): Promise<void> {
    await tx.query(
      `insert into public.crew_events (room_id, revision, actor_user_id, command_type, payload)
       values ($1::uuid, $2::bigint, $3::uuid, $4::text, $5::text::jsonb)`,
      [roomId, revision, actor, type, JSON.stringify(payload ?? {})],
    );
  }

  /** Keeps public.crew_mission_attempts in sync whenever begin()/removePlayer()
  ...
  private async syncMissionAttempt(tx: DbTx, roomId: string, prev: State, next: State): Promise<void> {
    if (next.attemptId && next.attemptId !== prev.attemptId) {
      if (prev.attemptId)
        await tx.query(
          `update public.crew_mission_attempts set status = 'abandoned', finished_at = now()
           where attempt_id = $1::uuid and status = 'active'`,
          [prev.attemptId],
        );
      await tx.query(
        `insert into public.crew_mission_attempts (attempt_id, room_id, mission_id, attempt_number, ruleset_version, status)
    ...
    if (next.attemptId && ["success", "failure"].includes(next.phase) && next.phase !== prev.phase)
      await tx.query(
        `update public.crew_mission_attempts set status = $2::text, finished_at = now() where attempt_id = $1::uuid`,
  ...
  private async requireRoom(tx: DbTx, roomId: string, forUpdate = false): Promise<void> {
    const rows = await tx.query<Row>(
      `select id from public.crew_rooms where id = $1::uuid${forUpdate ? " for update" : ""}`,
      [roomId],
    );
  ...
  private async requireMember(tx: DbTx, roomId: string, actor: string): Promise<void> {
    const rows = await tx.query<Row>(
      `select 1 from public.crew_room_members where room_id = $1::uuid and user_id = $2::uuid`,
      [roomId, actor],
    );
  ...
  async create(actorAuthId: string, input: CreateRoom) {
    return this.pool.begin(async (tx) => {
      ...
      await tx.query(`insert into public.crew_room_members (room_id, user_id) values ($1::uuid, $2::uuid)`, [state.roomId, actorAuthId]);
      await tx.query(`insert into public.crew_game_states (room_id, state) values ($1::uuid, $2::text::jsonb)`, [state.roomId, JSON.stringify(state)]);
  ...
  async join(actorAuthId: string, input: JoinRoom) {
    return this.pool.begin(async (tx) => {
      ...
      const [room] = await tx.query<Row>(`select id from public.crew_rooms where invite_token_hash = $1::text`, [inviteHash]);
      ...
      if (isMember) {
        ...
        await tx.query(`insert into public.crew_room_members (room_id, user_id) values ($1::uuid, $2::uuid)`, [roomId, actorAuthId]);
        await tx.query(`update public.crew_rooms set revision = $2::bigint, phase = $3::text, updated_at = now() where id = $1::uuid`, [
          roomId, nextState.revision, nextState.phase,
        ]);
        await tx.query(`update public.crew_game_states set state = $2::text::jsonb, updated_at = now() where room_id = $1::uuid`, [
  ...
  async command(actorAuthId: string, roomId: string, input: Envelope): Promise<Snapshot> {
    return this.pool.begin(async (tx) => {
      ...
      await tx.query(
        `update public.crew_rooms set revision = $2::bigint, phase = $3::text, attempt_id = $4::uuid, updated_at = now() where id = $1::uuid`,
  ...
  async invite(actorAuthId: string, roomId: string, commandId: string): Promise<{ inviteToken: string }> {
    return this.pool.begin(async (tx) => {
      ...
      const [room] = await tx.query<Row>(`select host_user_id from public.crew_rooms where id = $1::uuid for update`, [roomId]);
      ...
      const inviteToken = randomToken();
      const inviteHash = await sha256Hex(inviteToken);
      await tx.query(`update public.crew_rooms set invite_token_hash = $2::text, updated_at = now() where id = $1::uuid`, [roomId, inviteHash]);
  ...
  async leave(actorAuthId: string, roomId: string): Promise<void> {
    await this.pool.begin(async (tx) => {
      ...
      await tx.query(`delete from public.crew_room_members where room_id = $1::uuid and user_id = $2::uuid`, [roomId, actorAuthId]);
      await tx.query(
        `update public.crew_rooms set revision = $2::bigint, phase = $3::text, attempt_id = $4::uuid, host_user_id = $5::uuid, updated_at = now() where id = $1::uuid`,
