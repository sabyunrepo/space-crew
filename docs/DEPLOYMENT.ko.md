...
공개 주소에서 4인 테스트가 목표 선택 단계에서 멈췄다. 차례인 대원의 화면이 자기 차례를 모르는 상태였다. private Broadcast는 가끔 누락되는데, 클라이언트에 주기적 재조회가 없었고 스냅샷 캐시에 유효 시간도 없어서 그 화면이 영원히 옛 상태를 보고 있었다.

수정(`src/services/supabase.ts`, 배포 `9155ad9`):
- 구독 중 4초마다 스냅샷을 다시 조회한다. 그 사이 방송을 받았으면 건너뛴다. 탭이 숨겨져 있으면 쉬고, 다시 보이면 즉시 조회한다.
...
- 전환 순서와 시각: 방화벽 먼저 → `spacecrew2` 일시정지 → 허락 명단 제자리 수정 → 설정 재생성 → 시작(약 64초 중단(15:47:20~15:48:24 KST, 외부 접속 확인 완료 기준), 11단계 전부 성공) → 역할 권한 부여 → Coolify 환경변수 → 강제 재빌드 배포(15:56 KST, `/api/crew` 응답이 404에서 401로 한 번에 바뀜).
- 배포 커밋: `main` = `38d8b1a` (코드 커밋 `dc6fac1`, `d062c47`). 비밀값은 이미지에 없다(`docker history`에서 `CREW_` 0건). 이후 버그 수정 2건이 PR #3으로 병합되어 `main` = `b50cd44`가 운영 중이다.
...
- 배포 중 끊김: Traefik 헬스체크 라벨 적용 후 502는 0건. 다만 라벨이 같은 상태의 배포 3회 중 측정한 2회 모두 컨테이너 교체 순간에 요청 1건이 연결 실패(000)로 끊겼다. 원인은 확인되어 2026-09-21 커밋 `691201e`에서 수정했다: SIGTERM을 받으면 `/healthz`를 즉시 503으로 돌려 Traefik이 컨테이너를 빼 가게 하고, 4초 뒤 새 연결만 막고, 처리 중인 요청을 마친 뒤 DB 연결을 닫는다(전체 8초 마감). 수정 후 재배포 1회 측정: 컨테이너 교체 구간을 포함한 43회 동안 `/api/crew` 응답은 전부 정상(401), 연결 실패 0건. 같은 구간에 `/healthz`가 503을 1회 돌려줬는데, 이는 내려가는 컨테이너가 의도대로 "곧 종료"를 알린 것이다. 표본이 적으므로 다음 배포들에서 계속 확인한다.
- 방을 나갈 때의 규칙 오류: 원인은 나가기 규칙이 아니라 오류 처리였다. 핸들러가 `instanceof ApiError`로 판정했는데 `ApiError` 클래스 사본이 둘(`shared/contracts.ts`와 `supabase/functions/_shared/contracts.ts`)이라 저장소·엔진이 던진 규칙 오류가 전부 500으로 나갔다(Edge Function과 Node 서버 양쪽). 2026-09-21 커밋 `85ede24`에서 모양으로 판정하게 고쳤다. 운영 확인: 마지막 대원 나가기 409 `LAST_MEMBER`, 이미 나간 사람 403 `NOT_MEMBER`, 남의 방 조회 403, 없는 방 404 `ROOM_NOT_FOUND`. 규칙은 원래대로 "마지막 대원은 방을 나갈 수 없다"이다. 되돌리기용 Edge Function에는 아직 반영되지 않았다(`sbp functions deploy`로 따로 재배포해야 한다).
