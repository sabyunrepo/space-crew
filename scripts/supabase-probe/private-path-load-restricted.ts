/** Private-path probe under the RESTRICTED role: same measurements as
 * private-path-load.ts, but connecting as sbp_app_runtime (private DB
 * endpoint spec, 2026-09-21) instead of an admin role. sbp_app_runtime has no
 * grant on auth.* at all, so this variant never inserts into auth.users --
 * it takes existing user ids from PROBE_USER_IDS (created ahead of time
 * through the project's real Auth API as anonymous sign-ups) and cycles
 * through that pool for room membership. This is the proof that the real
 * PostgresRepository (crew tables + realtime.send + FK to auth.users) works
 * end to end under the role a private-network game server would actually use.
 *
 * Bundle with esbuild and run on the Coolify VM:
 *   DATABASE_URL=postgres://sbp_app_runtime:...@172.25.0.154:<port>/postgres \
 *   PROBE_USER_IDS=uuid1,uuid2,... node private-path-load-restricted.mjs [poolMax] [prepare:0|1]
 *
 * Do not edit private-path-load.ts; this is a separate file so the original
 * admin-role probe stays untouched. Run only against a disposable project. */
...
const userIdsRaw = process.env.PROBE_USER_IDS;
if (!userIdsRaw) throw new Error("PROBE_USER_IDS required (comma-separated existing auth.users ids)");
const userPool = userIdsRaw.split(",").map((s) => s.trim()).filter(Boolean);
if (userPool.length < 45) throw new Error(`PROBE_USER_IDS needs at least 45 ids, got ${userPool.length}`);
...
// Cycle through the real, pre-created user pool instead of minting synthetic
// auth.users rows -- sbp_app_runtime cannot write to auth.* at all.
let cursor = 0;
const nextMembers = (players: number) => {
  const picked: string[] = [];
  for (let i = 0; i < players; i++) picked.push(userPool[(cursor + i) % userPool.length]);
  cursor += players;
  return picked;
};
const settings: Settings = { name: "private-path-restricted", capacity: 5, missionMode: "sequential", startMission: 1 };
let seq = 0;
const run = Date.now() % 100000;
const nextCommandId = () => `2${String(run).padStart(7, "0")}-0000-4000-8000-${String(900000 + seq++).padStart(12, "0")}`;

async function makeRoom(label: number, players: number) {
  const [host, ...guests] = nextMembers(players);
  const { snapshot, inviteToken } = await repo.create(host, { commandId: nextCommandId(), nickname: `호스트${label}`, settings });
  const members = [host];
  for (let i = 0; i < guests.length; i++) {
    await repo.join(guests[i], { commandId: nextCommandId(), nickname: `대원${i + 1}`, inviteToken: inviteToken! });
...
console.log(`pool max=${poolMax} prepare=${prepare} role=sbp_app_runtime users=${userPool.length}`);
await sql.end();
